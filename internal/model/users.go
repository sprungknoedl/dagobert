...
func (u *User) String() string { return fmt.Sprintf("%s (%s)", u.Name, u.Login) }
...
func (store *Store) GetUserByLogin(login string) (User, error) {
	obj := User{}
	tx := store.DB.First(&obj, "login = ?", login)
	return obj, tx.Error
}
