...
func (store *Store) ListEvidences(cid string) ([]Evidence, error) {
	query := `
	SELECT id, type, name, hash, size, source, notes, case_id
	FROM evidences
	WHERE case_id = :cid
	ORDER BY name ASC`
...
func (store *Store) SaveEvidence(cid string, obj Evidence) error {
	query := `
	INSERT INTO evidences (id, type, name, hash, size, source, notes, case_id)
	VALUES (iif(:id != '', :id, lower(hex(randomblob(5)))), :type, :name, :hash, :size, :source, :notes, :cid)
	ON CONFLICT (id)
